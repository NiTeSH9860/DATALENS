...
function Dashboard({ analysis, conversation, question, busy, onQuestion, onSubmit }: { analysis: Analysis; conversation: { question: string; answer: Answer }[]; question: string; busy: boolean; onQuestion: (value: string) => void; onSubmit: () => void }) {
  ...
  return <>
    ...
    <div className="panel"><div className="panel-head"><div><p className="eyebrow">02 / Inspect</p><h2>Automatic visual insights</h2></div><span className="badge">Generated on upload</span></div><div className="charts">{analysis.visualizations?.map((chart) => <Chart key={chart.title} chart={chart} />)}</div></div>
...
function Chart({ chart }: { chart: Visualization }) {
  ...
  return <article className="chart-card"><h3>{chart.title}</h3><div className="bars">{chart.values.map((value, index) => <div className="bar-item" key={`${chart.labels[index]}-${index}`}><div className="bar-track"><div className={`bar-fill ${chart.kind === 'histogram' ? 'orange' : ''}`} style={{ height: `${Math.max((value / max) * 100, 5)}%` }} /></div><span title={chart.labels[index]}>{chart.labels[index]}</span><b>{value}</b></div>)}</div></article>
}
